...
#define CHECK_CUDA_ERROR(val) check((val), #val, __FILE__, __LINE__) // MACRO
template <typename T>
...
__global__ void kernel1(float *data, int n) {
    ...
    if (idx < n) {
        data[idx] *= 2.0f; // just multiplying by 2
    }
}
...
int main(void) {
    ...
    float *h_data, *d_data; // defining variables, setting N, etc - stuff weve seen 
    cudaStream_t stream1, stream2; // declaring our streams
    cudaEvent_t event; // inir our event 
    std::cout << event << std::endl; // printimg out our event 
    ...
    // essentially giving us more control over the concurrent execution of our kernels 
    ...
    // Create event
    CHECK_CUDA_ERROR(cudaEventCreate(&event)); // creating event using prev event type that we defined 
    ...
    CHECK_CUDA_ERROR(cudaEventRecord(event, stream1)); // we have to wait for events to trigger first 
                                                        // to then do our kernel execution 
    ...
    CHECK_CUDA_ERROR(cudaMemcpyAsync(h_data, d_data, size, cudaMemcpyDeviceToHost, stream2)); // copy back to host

    // Synchronize streams
    CHECK_CUDA_ERROR(cudaStreamSynchronize(stream1)); // to finalize, we always want to syncronize our streams
